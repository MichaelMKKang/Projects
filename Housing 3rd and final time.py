# -*- coding: utf-8 -*-
"""
Created on Sun Apr  2 19:46:41 2017
...
#This is for trying to remove all houses with GrLivArea greater than 4000
...
score = (score_ridge + score_lasso + score_EN + score_forest + score_GBest + score_xgb)/6
scores = (score_ridge + score_lasso + score_EN+ score_GBest + score_xgb)/5

'''
#Neural Network
X_trained = X_train.values.astype(float)
Y_trained = Y_train.values
#Creating model
model = Sequential()
model.add(Dense(10, input_dim=X_trained.shape[1], kernel_initializer='normal', activation='relu'))
...
neural_history = model.fit(X_trained, Y_trained, validation_split=0.33, nb_epoch=50, batch_size=5, callbacks=callbacks_list)
plt.plot(neural_history.history['loss'])
plt.plot(neural_history.history['val_loss'])
...
#pd.DataFrame({'Id': np.arange(Y_pred.shape[0])+1461, 'SalePrice': Y_pred}).to_csv('C:/Users/Michael Kang/Desktop/Data_Files/Housing/Housing_predictions.csv', index =False) 


'''
ridge = Ridge(alpha = 5)
cv = KFold(n_splits=5,shuffle=True,random_state=42)
results_ridge = cross_val_score(ridge, X_train, Y_train, cv=cv, scoring='neg_log_loss')
...
'''
cv = KFold(n_splits=5,shuffle=True,random_state=42)
cv_ridge = [cross_val_score(Ridge(alpha = alpha), X_train, Y_train, cv).mean() for alpha in alphas]
